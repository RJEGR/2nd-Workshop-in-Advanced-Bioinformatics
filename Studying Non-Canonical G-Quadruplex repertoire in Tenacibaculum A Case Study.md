...
```bash
ssh gricardo@kneipe.lavis.unam.mx
# 9MMytH2Bxc
```
...
Randomize (shuffle) Genome Simulations for 26 genomes

4. Test **G4PromFinder** for G4 based promoter prediction
...
Modify the G4PromFinder.py in order to make a loop:
...
about_promoter_filename = File_genome+"_about.txt"
promoter_coordinates_filename  = File_genome+"_coordinates.txt"
...
try:
    ...
    NO1 = 0
    NO2 = 0
    q = open(promoter_coordinates_filename,"a")
    ...
    with open(about_promoter_filename, "a") as file:
...
```bash
scp -r srohit@kneipe.lavis.unam.mx:/home/srohit/unamworkshop2021/raw_data/coordinates_files/
```
...
> https://www.biostars.org/p/69756/
